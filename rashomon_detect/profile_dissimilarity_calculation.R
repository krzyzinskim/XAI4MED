calculate_distances <- function(profiles_list, pdi_method=derivative_fraction_sign_difference){
  num_models <- length(profiles_list)
  all_vnames <- unique(unlist(lapply(profiles_list, function(df) unique(df$agr_profiles$`_vname_`))))
  num_vars <- length(all_vnames)
  
  # Create an empty matrix to store the distances
  dist_matrix <- matrix(0, nrow = num_models*(num_models-1)/2, ncol = 2 + num_vars,
                        dimnames = list(NULL, c("model_ind_1", "model_ind_2", all_vnames)))
  ...
  for (i in 1:(num_models-1)) {
    for (j in (i+1):num_models) {
      # Get the profiles for model i and model j
      ...
      for (k in 1:num_vars) {
        ...
        # Get the profiles for variable k in model i and model j
        profiles_i_var <- profiles_i[profiles_i$`_vname_` == vname, "_yhat_"]
        profiles_j_var <- profiles_j[profiles_j$`_vname_` == vname, "_yhat_"]
        profiles_var_vals <- profiles_i[profiles_i$`_vname_` == vname, "_x_"]
        ...
        distance <- pdi_method(profiles_i_var, profiles_j_var, profiles_var_vals)
...
get_final_distance_matrix <- function(dist_matrix, selected_models_indices){
  num_selected_models <- length(selected_models_indices)
  dist_matrix_selected <- matrix(0, nrow = num_selected_models, ncol = num_selected_models)
  ...
  for (i in 1:(num_selected_models - 1)) {
    for (j in (i + 1):num_selected_models) {
      ...
      avg_distance <- dist_matrix$avg_pdi[(dist_matrix$model_ind_1 == model_ind_1 &
